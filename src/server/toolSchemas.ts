...
export function getLuaToolSchemas(): any[] {
  return [
    ...
    },
    {
      ...
      description: "Preview or apply passive tree allocation changes. By default this asks the PoB bridge to apply the delta, return the resulting tree, and restore the loaded build immediately; set apply=true only when you intentionally want to keep the stateful mutation. The handler snapshots the node list before the preview/apply operation and reports actual added/removed/dropped nodes returned by PoB import. Note: max 8 ascendancy points allowed.",
      inputSchema: {
        type: "object",
        properties: {
          add_nodes: {
...
export function getSkillGemToolSchemas(): any[] {
  return [
    ...
    },
    {
      ...
      description: "Generate a prioritized gem upgrade checklist showing which gems to level, quality, corrupt, or check for Exceptional versions. Does not live-check prices or acquisition; Gem quality/corruption/Exceptional availability is version-sensitive, so verify with current PoB gem data and post-change stat readback before buying or corrupting. Corruption can add at most +1 level or +3 quality.",
      inputSchema: {
        type: "object",
        properties: {
          build_name: {
